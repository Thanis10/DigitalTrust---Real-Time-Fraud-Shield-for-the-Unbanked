...
import { Lock, ShieldCheck, X, Camera } from 'lucide-react';
              ...
              {method === 'face' ? (
                <div className="space-y-8 py-4">
                  <div className="relative mx-auto w-40 h-40">
                    ...
                    <motion.div animate={{ top: ['0%', '100%', '0%'] }} transition={{ duration: 2, repeat: Infinity, ease: "linear" }} className="absolute left-0 right-0 h-1 bg-indigo-400 shadow-[0_0_15px_rgba(129,140,241,0.8)] z-10" />
                  </div>
                  <div className="space-y-2">
                    <p className="text-indigo-400 font-black uppercase tracking-widest text-[10px]">
                      {isScanning ? 'Scanning Face...' : 'Identity Confirmed'}
                    </p>
                    <button onClick={() => setMethod('pin')} className="text-[10px] text-slate-500 hover:text-slate-300 font-black uppercase tracking-widest transition-colors">Use Secret PIN Instead</button>
                  </div>
                </div>
              ) : (
                <form onSubmit={handlePinSubmit} className="space-y-8 py-4">
                  <div className="flex justify-center gap-4">
                    {[0, 1, 2, 3].map((i) => (
                      <div key={i} className={`w-4 h-4 rounded-full border-2 transition-all ${pin.length > i ? 'bg-indigo-500 border-indigo-500' : 'border-white/20'}`} />
                    ))}
                  </div>
                  <div className="grid grid-cols-3 gap-4 max-w-[240px] mx-auto">
                    {[1, 2, 3, 4, 5, 6, 7, 8, 9, 'C', 0, '✓'].map((val) => (
                      <button key={val.toString()} type="button" onClick={() => { if (val === 'C') setPin(''); else if (val === '✓') { if (pin.length === 4) onVerify(); } else if (pin.length < 4) setPin(p => p + val); }} className="w-16 h-16 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center text-xl font-black text-white hover:bg-white/10 transition-colors active:scale-95">{val}</button>
                    ))}
                  </div>
                  <button type="button" onClick={() => setMethod('face')} className="text-[10px] text-slate-500 hover:text-slate-300 font-black uppercase tracking-widest transition-colors">Switch to Face Scan</button>
                </form>
              )}
