...
export default function SecureVaultCard({ onMoveMoney, onVerifyClick }: SecureVaultCardProps) {
  const { vaultBalance, locationCurrency, isUserVerified, t } = useWalletStore();
  ...
  return (
    <motion.div
      ...
      className="relative rounded-[2.5rem] p-6 bg-gradient-to-br from-slate-900 to-indigo-950 border border-indigo-500/30 overflow-hidden group shadow-2xl h-full"
    >
      ...
      <div className="relative z-10">
        <div className="flex justify-between items-center mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-black text-white">{t('secure_vault')}</h3>
              <div className="flex items-center gap-1">
                <ShieldCheck className="w-3 h-3 text-emerald-400" />
                <span className="text-[10px] font-black text-emerald-400 uppercase tracking-widest">{t('stricter_rules')}</span>
              </div>
            </div>
          </div>
        ...
        <div className="space-y-1 mb-6">
          <div className="flex items-center justify-between">
            <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">{t('savings_protected')}</p>
            <button 
        ...
        <div className="grid grid-cols-2 gap-3 relative z-30">
          <button 
            ...
            className="py-3.5 px-4 bg-indigo-500 rounded-2xl font-black text-[10px] uppercase tracking-widest text-white flex items-center justify-center gap-2 shadow-lg active:scale-95 transition-all"
          >
            {t('move_in')}
          </button>
          <button 
            onClick={(e) => { e.stopPropagation(); onMoveMoney('withdraw'); }}
            className="py-3.5 px-4 bg-white/5 border border-white/10 rounded-2xl font-black text-[10px] uppercase tracking-widest text-slate-300 flex items-center justify-center gap-2 active:scale-95 transition-all"
          >
             {t('withdraw')}
          </button>
        </div>
      ...
      {!isUserVerified && (
        <div 
          onClick={(e) => { e.stopPropagation(); onVerifyClick?.(); }}
          className="absolute inset-0 bg-slate-950/40 backdrop-blur-[2px] z-[35] flex items-center justify-center cursor-pointer group/overlay"
        >
          <motion.div 
            whileHover={{ scale: 1.05 }}
            className="bg-slate-900/90 border border-white/20 px-5 py-2.5 rounded-full flex items-center gap-3 shadow-2xl"
          >
            <div className="w-6 h-6 rounded-full bg-amber-500/20 flex items-center justify-center border border-amber-500/30">
               <Lock className="w-3.5 h-3.5 text-amber-400" />
            </div>
            <span className="text-[10px] font-black text-white uppercase tracking-[0.1em]">{t('verification_req')}</span>
          </motion.div>
        </div>
      )}
    </motion.div>
